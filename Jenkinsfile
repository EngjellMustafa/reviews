...
pipeline {
  ...
  stages {
    ...
    stage("Create New Pods") {
      steps {
         withKubeCredentials(credentials: [[ caCertificate: '', clusterName: 'minikube', contextName: 'minikube', credentialsId: 'jenkins-k8s-token', namespace: '', serverUrl: 'https://172.22.18.25:8443'
        ]])
        {
          script {
